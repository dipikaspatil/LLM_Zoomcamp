...
- Create and activate virtual envirnment - `python3 -m venv .venv &&  source .venv/bin/activate`
- Install `uv init && uv add gitsource`
- Create file and run `python3 q1_download_lesson_pages.py`

Output 
```bash
01_agentic_rag % python3 q1_download_lesson_pages.py
...
- 01-agentic-rag/lessons/14-agentic-loop.md <-- answer
- 04-evaluation/lessons/13-llm-as-judge.md
- 06-best-practices/lessons/02-hybrid-search.md

### Steps followed - 
- Install minsearch `uv add minsearch`
- Create file and run `python3 q2_index_documents.py`

# Output
```bash
01_agentic_rag % python3 q2_index_documents.py
...
### Steps followed
- Download and update `rag_helper.py` for markdown documents
- Create file and run `q3_rag_assitant.py`

Output
```
01_agentic_rag % python3 q3_rag_assitant.py
...
With size=2000 and step=1000: # Implementaiton - https://github.com/alexeygrigorev/gitsource/blob/master/gitsource/chunking.py
    ...
```
...
### Step followed:
- Create and run `q4_chunk_documents.py`


Output
```
01_agentic_rag % python3 q4_chunk_documents.py
...
### Steps followed - 
- Create file and run `q5_rag_with_chunking.py`

Output
```
01_agentic_rag % python3 q5_rag_with_chunking.py
Answer: The loop keeps calling the model inside a `while True` loop. After each model response, it checks whether any `function_call` items were returned:

- If there are function calls, it executes them, appends the results to the message history, and loops again.
...
Input tokens (Q3 full docs): 7110
Ratio: 3.1 × fewer
```
...
### Steps followed
- Install `uv add toyaikit`
- Create file and run `q6_hand_written_agentic_loop.py`

Output

```
01_agentic_rag % python3 q6_hand_written_agentic_loop.py
  [Search #1] Query: agentic loop RAG course explanation
  [Search #2] Query: plain RAG vs agentic loop
  [Search #3] Query: agentic retrieval loop tool use reflection course

Answer: The **agentic loop** is the repeated cycle where the LLM:

1. gets the user question plus the conversation history,
2. decides whether it needs a tool,
3. if yes, calls the tool,
4. receives the tool result,
5. sends that result back into the model,
6. repeats until it can produce a final answer.

In the course, this is described as the agent being a loop with three core ingredients:

- **instructions**: the role/behavior you want,
- **tools**: functions it can call,
- **memory**: the message history it keeps appending to.
...
**Plain RAG** is usually a one-pass pipeline:
...
So the flow is fixed:

- one search,
- one prompt construction,
- one LLM call.

### Key difference

- **Plain RAG**: retrieval happens once, before generation.
- **Agentic loop**: the model can retrieve multiple times, change strategy, and use tool results to decide what to do next.

### Why the agentic loop is more powerful

The course notes that plain RAG breaks down when:

- the first search misses the answer,
- the model needs to search several times,
- the next step depends on the previous tool result.

The agentic loop handles that because the LLM can:
- try a search,
- inspect the result,
- refine the query,
- search again,
- and only stop when it’s ready.

### Tradeoff

Agentic loops are more flexible, but they also cost more because each tool-using turn can require multiple LLM calls and extra history tokens.

So in one sentence:

**Plain RAG is single-shot retrieval + generation; the agentic loop is iterative tool use + reasoning + retrieval until the model decides it’s done.**

If you want, I can also sketch the exact flow of messages in the loop step by step.

Search tool called 3 times
```

- Create file and run `q6_agent_with_toyaikit.py`

Output
```
01_agentic_rag % python3 q6_agent_with_toyaikit.py
  [Search #1] Query: agentic loop RAG tool use planning reflection course
  [Search #2] Query: plain RAG retrieval generation difference agentic loop
  [Search #3] Query: agentic workflow loop think act observe retrieve generate course
